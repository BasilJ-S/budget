...
import yaml
from clean import Ruleset, build_shorthand_and_list, RULESET_PATH, load_ruleset
...
def save_budget(budget: Budget):
    write_budget(budget)
    write_budget(budget, filename=f'backup_{budget.name}_{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}')
...
def list_budgets() -> None:
    ...
    budgets = [f[:-5] for f in files if f.endswith('.yaml') and not f.startswith('backup')]
    return budgets
...
def edit_budget_metadata(budget: Budget) -> Budget:
    budget.name = input(f"Enter new budget name (current: {budget.name}): ") or budget.name
    start_date_str = input(f"Enter new start date (YYYY-MM-DD) (current: {budget.start_date}): ")
    if start_date_str:
        budget.start_date = dt.datetime.strptime(start_date_str, "%Y-%m-%d").date()
    end_date_str = input(f"Enter new end date (YYYY-MM-DD) (current: {budget.end_date}): ")
    if end_date_str:
        budget.end_date = dt.datetime.strptime(end_date_str, "%Y-%m-%d").date()
    total_budget_str = input(f"Enter new total budgeted amount (current: {budget.total_budgeted}): ")
    if total_budget_str:
...
def print_budget_items(budget: Budget):
    ...
    print("Current items:")
    budget_left = budget.total_budgeted - sum(item.budgeted_amount for item in budget.items)
    print(f"Total budget: {budget.total_budgeted}, Total budgeted: {sum(item.budgeted_amount for item in budget.items)}, Budget left: {budget_left}")
    for idx, item in enumerate(budget.items):
        print(f"{idx + 1}. Categories: {item.categories}, Amount: {item.budgeted_amount}")
...
def edit_budget(budget: Budget, ruleset: Ruleset) -> Budget:
    ...
    while True:
        action = input("Would you like to add, edit, or remove an item? Or modify budget metadata? (a/e/r/m/done): ").strip().lower()
        if action == 'a':
...
if __name__ == "__main__":
    ...
    new_or_edit = input("Would you like to create a new budget or edit an existing one? (new/edit): ").strip().lower()
    if new_or_edit == 'edit':
